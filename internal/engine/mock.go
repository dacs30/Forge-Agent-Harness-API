...
type MockEngine struct {
	...
	ExtractArchiveFn      func(ctx context.Context, containerID, destDir string, tarStream io.Reader) error
	SnapshotContainerFn   func(ctx context.Context, containerID, snapshotID string) (string, error)
	DeleteSnapshotImageFn func(ctx context.Context, imageID string) error

	// ExtractedArchives records destDir arguments passed to ExtractArchive, in
...
func (m *MockEngine) ExtractArchive(ctx context.Context, containerID, destDir string, tarStream io.Reader) error {
	m.ExtractedArchives = append(m.ExtractedArchives, destDir)
	if m.ExtractArchiveFn != nil {
		return m.ExtractArchiveFn(ctx, containerID, destDir, tarStream)
	}
	return nil
}

func (m *MockEngine) SnapshotContainer(ctx context.Context, containerID, snapshotID string) (string, error) {
	if m.SnapshotContainerFn != nil {
		return m.SnapshotContainerFn(ctx, containerID, snapshotID)
	}
	return "mock-snapshot-image-id", nil
}

func (m *MockEngine) DeleteSnapshotImage(ctx context.Context, imageID string) error {
	if m.DeleteSnapshotImageFn != nil {
		return m.DeleteSnapshotImageFn(ctx, imageID)
	}
	return nil
}
